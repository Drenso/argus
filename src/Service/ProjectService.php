...
class ProjectService
{
  ...
  public function createMergeRequest(?Project $project)
  {
    ...
    $this->gitlabApiConnector->projectApi($project, 'POST', 'merge_requests', [
        'json' => [
            ...
            'assignee_id'   => $this->gitlabInstanceDetailsService->getDefaultMrAssigneeIdForProject($project),
        ],
    ]);
  }
  ...
  public function getOutdated(): array
  {
    ...
    foreach ($this->gitlabInstanceDetailsService->getConfiguredHosts() as $host) {
      $projects = $this->gitlabApiConnector
          ->gitlabApi($host, 'GET', '?simple=true&archived=false&per_page=100&order_by=last_activity_at');

      foreach ($projects as $project) {
        $projectObj = $this->projectRepository->findOneByNameAndHost(
            $this->propertyAccessor->getValue($project, '[path_with_namespace]'), $host
        );
        if (!$projectObj) {
          continue;
        }
        ...
        $result[] = [
            ...
            'gitlab_diff_url' => $this->gitlabInstanceDetailsService->getProjectDiffUrl($projectObj, 'master', 'production'),
        ];
      }
    }
